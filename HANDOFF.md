...
5. **[Both] The 35 continuous databases / beam decision.** [Agent] gives the 35
   ...
   contradiction and keeps `undefined` meaning one-off instrument. [Thomas]
   decides whether the beam rendering (with a scrolling gradient for direction)
   goes into Phase 2/3 scope or waits.
6. **[Agent] Phase 3 — only if Phase 2 holds up.** Geography-takes-the-fill
   mode; typed edges (remember: `methodology_depends_on` is the MOST common type
   at 407, and a trunk aggregates up to 57 mixed edges — answer what a trunk's
   "type" means first).
7. **[Both] Phase 4 backlog** — `notes/phase-4-brief-2026-08-19.md`. [Thomas]
   ranks what bothers him most; [Agent] builds. Candidate order from the brief:
   delete Blueprint (subtractive, cheapens everything after — but first spend
   ten minutes learning why it looked broken), fix the key light sitting inside
   the graph, double edge lengths, hover/click affordances, the edge evidence
   card (the project's best material is currently thrown away before drawing),
   menu bar, help wiring, saved views.
