...
Last updated: 2026-09-08 ~03:25 UTC (still handoff 075 — §2/§3 edited in place for
rounds 9 through 22; not a new handoff under §4, no archive)
...
Corpus **3,579 reports / 3,143 dependencies**. **1,132 A · 1,399 B · 612 C**, A-share
36.0%. **Domains: 46 approved, 0 proposed.** `validate` exits 0, **128/128 logic tests**,
`tsc --noEmit` clean, `vite build` ok,
`public/corpus-data.json` regenerated and copied back, current as of 2026-09-08
~03:25 UTC. **980 nodes still have zero edges** — unchanged (rounds 18-21's new nodes
...
**The e-GDDS third of the tier finding: 25 of the original 34 now wired, 9 open (corrected count — see below).** Botswana (round 19), Tanzania and Côte d'Ivoire (round 20), 18 more in round 21, then Burundi/Djibouti/Guinea/Central African Republic in round 22 (all of round 21's "group 1"). **The method (search "<country> opendataforafrica NSDP"/read the country's own NSDP page in Chrome; if the portal's own nav has no NSDP tab, check the IMF's e-GDDS launch press release for that country instead — it links straight to the real page) is now proven on 25 of 26 countries tried** across four rounds; only Botswana needed a different fallback (its own opendataforafrica.org page was 403'd even in Chrome; wired off the Bank of Botswana's own page instead).
**Count correction (round 22):** round 21's own write-up said 13 remained in two named groups (4 + 7 = 11) — arithmetic that never added up. Cross-checked against the original 34-country list in `int-imf-tier-sweep-2026-09-07.json`'s `_dropped`: **Guyana and Serbia are on that list and were never assigned to either group.** With group 1 now closed, **9 remain: Andorra, Albania, Micronesia, Montenegro, Palau, San Marino, Kosovo (round 21's group 2, not tried against opendataforafrica.org — the platform looks Africa-only on 26/26 evidence so far, go to IMF press release / DSBB directly), plus Guyana and Serbia (never checked by any route — try the press-release method first, same as this round).**
Detail: memory `round19_bw_nsdp_2026-09-07`, `round20_tz_ci_nsdp_2026-09-08`,
`round21_egdds_oda_sweep_2026-09-08`, `round22_egdds_group1_2026-09-08`. Publisher note for a future round: three of round 21's 18 (Guinea-Bissau, Mauritania, Somalia) and two of round 22's 4 (Burundi, Central African Republic) have their publisher field set from the NSO name already used elsewhere in-corpus or in the IMF's own press release, NOT independently confirmed on the NSDP page itself (its footer carries only generic AfDB/Knoema links) — worth a quick re-check if anyone doubts the attribution, though it does not affect the grade.
