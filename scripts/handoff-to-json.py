...
`CA/G.<n>.md`, `AF/G.<n>.md`) into a machine-readable sidecar (`<branch>/G.<n>.json`).
Branches: EU, NZ, AU, CA, AF — add new branch folder names to BRANCHES below.
...
def hook_mode() -> int:
    ...
    norm = path.replace("\\", "/")
    if not re.search(r"/(EU|NZ|AU|CA|AF)/G[._]\d+\.md$", norm):
        return 0
