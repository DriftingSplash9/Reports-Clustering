...
- **A signed download token is minted by the browser and then fetched by
  anyone** (2026-09-04). BPS serves its publication PDFs only through
  `web-api.bps.go.id/download.php?f=<token>`, generated by the landing page's
  own JavaScript; a cross-origin `fetch` of it from that landing page is
  CORS-blocked, which is what "the BPS PDFs cannot be transported" meant for
  two rounds. But the token is an ordinary URL once it exists: the **bridge
  VM curled all five, first try, 200 and 3.9–22.3 MB**. Mint the token in the
  browser (one `javascript_tool` call reading the Download link's `href`),
  then fetch it wherever the toolchain is. The same shape applies to any
  `download.php?token` host. Cross-origin is also beatable inside the browser
  by navigating to the FILE host first (`web-api.bps.go.id/robots.txt`, a 404
  is fine) and fetching same-origin from there.
- **pdf.js runs on the bridge VM and reproduces a Chrome in-page capture byte
  for byte** (2026-09-04). `npm i pdfjs-dist@3.11.174` outside the repo plus
  ~40 lines (`scripts/capture/extract-pdfjs.cjs`) gave 89,289 chars and
  sha256 `6b2db200…` for BPS Energy Balances — identical to what the same
  library produced inside the page, same offsets. **So a PDF never has to
  cross the bridge for its text.** Capture in Chrome only when the host
  refuses the VM outright (`anstat.ci` 403s it).
- **`pdftotext -layout` destroys a bilingual two-column PDF, and the symptom
  is a correct quote that exists in no extraction** (2026-09-04). BPS prints
  Indonesian and English side by side; `-layout` interleaves them word by
  word, so "Sumber data yang digunakan untuk penyusunan…" comes out with
  English clauses between its own words and no whitespace-insensitive matcher
  can help — the extra text is *inside* the span, not around it. Measured
  over the five BPS publications and 17 probe spans: `-layout` found 6, plain
  `pdftotext` found 6, **pdf.js reading order found 17**. The grader's own PDF
  branch is `-layout` (`getDoc`), so this is a whole class of
  `quote-not-in-document` that is a fact about the extractor, not the
  document. Do NOT rewrite the quote. Extract with pdf.js and land the record.
- **The checksummed transport, and the reason it exists** (2026-09-04,
  `scripts/capture/`). In the page: gzip the text, base64 it, sha256 the TEXT,
  emit ≤44,000-char slices into `document.body` as one `<main>`, read with
  `get_page_text`. Move the base64 by any channel and let `land.py` verify the
  sha before it writes. Compression is what makes it affordable — 38,084 chars
  of text is 17,512 chars of base64, usually one slice — and it makes round
  3's unreliable "over 50 KB and it persists to a file" trick unnecessary.
  **Emit in ~10,000-character chunks**: a 36,840-char heredoc was silently
  truncated mid-write in the session that built this, and the checksum is the
  only reason anyone noticed.
- **An extraction that produced nothing looks exactly like a document that
  says nothing.** A 0-byte pdf.js output was landed as a fulltext record
  without complaint on 2026-09-04, from a file that had read fine one call
  earlier. `land_text.py` now refuses anything under 200 chars. Check the byte
  count before building a record on an extraction.
- **`namesTarget` has no whitespace-insensitive pass and `locateQuote` does,
  so a PDF with broken word spacing can carry a quote at coverage 1.00 and
  still fail the naming test.** `ci-anstat-ihpc -> ci-anstat-ehcvm`: the PDF
  renders the survey's name as `Enquê te Harmonisé e sur lesConditions de Vie
  des Mé nages`, `tokenise` sees `enque te harmonise e lesconditions`, and the
  ≥60% title run can never fire although the document names the survey in
  full, twice. Third member of the family with the A-bar span anchor and the
  Hangul/`title_aliases` gap — same discipline: it can only add matches, so it
  owes a corpus-wide before/after before anyone changes it.
- **Take grade counts from `npm run validate`, never from
  `public/corpus-data.json`.** The generated file holds the 347 research
  slices only and misses the ~10 edges that live in the hand-written seed
  files (rule 11), so mixing the two produces a grade line that does not sum
  to the corpus. The 2026-09-04 handoff read "578 A · 1,343 B · 703 C" against
  2,634 dependencies; the real C was 713.
