...
**Every live edge carries an `evidence_grade`: 233 A · 1,332 B · 1,163 C**
across the 2,728 research-slice edges (the 8 seed-file edges are deliberately
never machine-written). A-share **8.5%**, up from 8.3%. `npm run validate`
exits 0 with all 233 A grades in — an A turns the three evidence warnings into
errors, so that is a real check. `tsc --noEmit` clean, 123/123 logic tests,
grader `--selftest` **28/28**.
...
422: 7 A · 131 B · 284 C (was 1 B · 421 C).

**Thomas ruled 2026-09-03: a document read from an archived copy caps at B**
(standing decision, PLAYBOOK §7 — it binds every future fetch strategy, not just
this one). 15 edges capped; the guard sits after the A bar so the bar itself is
untouched, with its own reason `quote-found-artefact-named-via-snapshot`. **All
7 surviving A grades were read directly.** Because `writeGrades` only writes
`evidence_quote` on an A, the 15 quotes the pre-ruling pass had written were
removed again (14 files) — a machine-written `evidence_quote` in this corpus
now always means "found in the live document".
...
`wall`/`network`/`empty` but **never a 404**, and whose reads cap at B; `Fetched.via` records the
...
2. Commit rounds 3a-3d. 3d touches: `scripts/grade-evidence.ts`, 101
...
3. Ruling **7**: **131 dead-URL edges**, 58 of them one dead host. Fix, drop, or
   leave graded C.
4. **Browser pass, re-scoped: 232 edges, and 59 of them are two hosts.**
...
6. Empty `_to_delete/` (now also holds a stale `.evidence-fulltext` store) and
...
4. Flip `view.minGrade` default to A — **after** 1-2, not before (8.5% A).
