...
Corpus **3,597 reports / 3,182 dependencies**. **1,169 A · 1,402 B · 611 C**, A-share
36.7%. **Domains: 46 approved, 0 proposed.** `validate` exits 0, **128/128 logic tests**,
...
(round 30 -- Jiangsu chapter notes + the first city edge, this session -- last data-changing
round).
**976 nodes still have zero edges** (CN: 31 of 75).
...
work is.** Current score: 2 provinces wired (both now fully mined), 8 node-but-unwired, 22 with no node
yet, and 1 of 10 cities wired.
...
**Round 30 (2026-09-08) did Jiangsu in depth and opened cities.** +1 node, +5 edges, all A.
Jiangsu's per-chapter 简要说明 (at `/2025/njNN/njNN00.htm`) gave three more instrument edges on
top of round 29's 编者说明 one, and **Nanjing gave the programme's first CITY edge** -- item 三 of
its own 编者说明 names GB/T 4754, exactly as a province does, so **a city reaches a national
standard directly and does not need the contested city-to-province hop** (Nanjing's edge to the
Jiangsu yearbook is quarantined as assertion-only in `cn-china-2026-08.json` `_dropped`). Also
minted `cn-nbs-city-development-monitoring-system`, the instrument NBS's 城市社会经济调查司
collects the CSY's city tables under -- a second hub every city node may be able to reach.
**Do cities alongside provinces from now on** (Thomas, 2026-09-08): same host family, same
one-line disclosure, one fetch each.
