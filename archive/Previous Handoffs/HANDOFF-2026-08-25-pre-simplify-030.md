...
16. **Pulse rate slider — SHIPPED, 2026-08-25 (Thomas: "a slider to adjust
    the 'time' so that pulse rate is easily toggleable").** New
    `ViewSettings.pulseRate` (default 1 = 100%, range 0–4 in the View
    panel's Pulse rate slider, same row style as Cluster spread/Geo-
    affinity/Galaxy pull). One multiplier, four things: the orb breath
    (`pulseClock` accumulation), the cross-border blink and the beam-flow
    shader (both driven off that same clock), and the travelling teardrop
    particles (`linkDirectionalParticleSpeed`, multiplied on top of the
    existing per-edge cadence from `pulseSpeed`). Read live off a ref —
    `pulseRateRef`, same pattern as `geoAffinity`/`galaxy` — so dragging it
    never rebuilds or re-warms the layout, only how fast the motion on top
    of it plays. 0 freezes everything in place without hiding it (a
    different thing from the existing Pulses toggle); confirmed live via
    Claude in Chrome — two screenshots of the same region 5s apart at 0%
    were pixel-identical, frozen. No schema bump needed for saved
    views/deep links — both merge over `DEFAULT_VIEW`, so an old save just
    picks up `pulseRate: 1`. Verified: `tsc --noEmit` clean, full sandbox
    `npm run validate`/`npm run build` clean. Full writeup:
    `notes/pulse-rate-slider-2026-08-25.md`.
