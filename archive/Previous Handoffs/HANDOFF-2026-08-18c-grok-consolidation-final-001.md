...
1. **Mint — decided yes, but deferred to its own session.** Thomas's ruling
   2026-08-18: he wants this minted, but it is too large to fold into other work
   and it is **last** on his current priority list. It needs a dedicated agent,
   not a spare half-hour at the end of a session. Do not start it as a side
   task. Read §7 first — it is still coupled to the palette.
...
5. **4 id collisions with the live corpus — policy now decided: live wins.**
   `in-mospi-cpi`, `in-rbi-balance-of-payments`, `ru-rosstat-cpi` and
   `ru-cbr-monetary-policy-guidelines` are defined in both the live corpus and the
   staged archive. Compared field by field on 2026-08-18: same concept, same
   publisher, same cadence, same jurisdiction in all four — they are genuinely the
   same report described twice. **The live record is better in every case** and
   the staged one is weaker precisely where it matters: `in-mospi-cpi`'s staged
   URL is `mospi.gov.in/`, a bare homepage the corpus rule rejects, against a live
   URL pointing at the actual rebasing press release; the staged RBI node uses a
   generic press-release script URL against a live one naming the Bulletin
   article; both Russian nodes stage a PDF against a live series landing page, and
   the live titles carry the native-language official name.
   **On mint: drop the staged duplicate, keep the live record.** One thing worth
   grafting rather than discarding — the staged RBI node tags `external-sector`,
   which is a better fit than the live `banking`.
...
The redundant copies are staged for deletion in
`Grok - Brics+israel and singapore/_to_delete/` — Thomas removes them, since an
agent session cannot delete. Moved there: `redundant-copies/` (the 40 loose
top-level batch JSONs, each verified byte-identical to its twin in
`grok-batches/raw/`, plus `candidates (1).zip`, a third copy of the same
archive), eight superseded `consolidated-pass*` folders, the transfer tarballs,
and `session-scratch/` (this session's working files from `tmp_work/audit/`).

**Kept deliberately:** `grok-batches/` is the raw provenance for the Latin
American and Asian material, and `brics-batches.zip`, `israel-batches (1).zip`
and `singapore-batches (1).zip` are the *only* copies of the raw BRICS, Israel
and Singapore batches. Do not clear those out with the rest.
...
**Thomas's beam idea (2026-08-18), and what it does and does not solve.** For the
35 continuously-updated databases, the proposal is to stop drawing discrete
teardrops and render the edge itself as a continuous lit beam — pulse and edge
becoming one object. Worth doing, for three reasons beyond the aesthetic: a solid
stream is honestly what infinite frequency looks like at the limit, so it says the
true thing; it is *cheaper* than particles, and Phase 0 already has to rebuild
`teardropGeometry` because `three-forcegraph` reads `.geometry` off the custom
particle object; and it removes a per-edge particle system on the corpus's fastest
edges.

Two things it does not solve, both worth deciding at the same time:

1. **Direction.** A travelling teardrop shows which way influence flows; a lit
   line does not. The beam needs an animated gradient or a scrolling UV offset to
   keep that cue, or the graph loses its only visual statement of direction on
   exactly the edges that matter most.
2. **The node, not the edge.** The beam fixes how a continuous source *transmits*.
   It does not fix that the node itself currently renders hollow — see §8 — because
   both behaviours come from the same absent `releases_per_year`. The cheapest
   honest fix is to give those 35 a real number (250 for business-daily, 365 for
   genuinely continuous — the corpus already uses 250 for daily FX), which keeps
   `undefined` meaning *only* "one-off instrument", which is what the renderer
   already assumes everywhere else. Then the beam is simply what a very high
   cadence looks like, and no schema change is needed at all.
...
- **`releases_per_year: undefined` is overloaded, and the renderer reads it two
  incompatible ways.** `isStandingInstrument()` in `nodeVisuals.ts` treats
  absence as *one-off instrument* and draws the node **hollow**;
  `InfluenceGraph.tsx` falls back to `upstream?.releases_per_year ?? 1` and pulses
  the edge **once a year**. For a treaty both are right. For the 35 staged nodes
  that are live continuously-updated databases both are wrong in opposite
  directions — they would render as hollow standing instruments trickling
  annually, when they are the fastest things in the corpus. types.ts already warns
  about exactly this confusion one field over (`ReferencePeriod`: "absent means
  unknown rather than continuous"). Fix before minting, and see §7's note on the
  beam idea.
