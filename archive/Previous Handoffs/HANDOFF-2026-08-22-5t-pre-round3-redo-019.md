...
Last written: **2026-08-22 (item 5t — Grok diary + prompt queue
established as §8: `notes/grok-diary.md` is now the mandatory read before
any Grok prompt, and round 3's prompt is written and queued. Same day,
item 5s — the cross-border mint LANDED; see §3)**.
...
`notes/grok-diary.md` FIRST (§8)**; regions/blocs/publishers →
...
### In flight
1. **Grok round 3 — prompt WRITTEN, awaiting Grok's reply.** See §8: the
   prompt is `notes/grok-prompt-cross-border-round3-2026-08-22.md` (Thomas
   pastes it with the attachments it lists); the playbook is
   `notes/grok-diary.md` — read it before touching any Grok prompt. When
   the reply lands: parse-check, raw-verify, mint (5r/5s template).
...
quotes, third-party scorecards). Append a dated lesson to the diary after
processing every Grok reply. Every handoff carries this pointer (Thomas's
standing instruction, 2026-08-22).

Prompts live beside the diary as `notes/grok-prompt-*.md`; the diary's
"Round log" section is the queue state. Currently queued: **round 3**
(`notes/grok-prompt-cross-border-round3-2026-08-22.md`, written 2026-08-22,
awaiting Thomas pasting it + the listed attachments into Grok) — asks for
the Mauritius SNA-2008 edge done properly (plus a possible
mu-national-accounts node), Afghanistan CR 06/251 corroboration, and
substantive cross-border edges for SD/SL/IQ/IR/AF/YE/SY. Whatever comes
back: parse-check the JSON, raw-verify every quote (rules 2/3), then mint —
the 5r/5s verify-then-mint flow is the template.
